...
@Component({
  selector: 'app-administrador-tipos-vehiculo',
...
export class TiposVehiculosComponent implements OnInit {
  ...
  rowSelected: string = '';
  public tipoVehiculoSelected: TipoVehiculo;
  private row;
  ...
  iniciarFormulariosParaModificar() {
    ...
    this.firstFormGroup = this.formBuilder.group({
      nombre: [this.tipoVehiculoSelected.nombre, Validators.compose([Validators.required, Validators.minLength(3), Validators.maxLength(40)])],
      descripcion: [this.tipoVehiculoSelected.descripcion, Validators.compose([Validators.minLength(3), Validators.maxLength(40)])]
    })
  ...
  loadImageFromService() {
    ...
    this.isImageLoading = true;
    this._imageService.getImage(this.tipoVehiculoSelected.imagen, 'tipo-vehiculo').subscribe(data => {
      console.log('Data: ');
      console.log(data);
      let extension = this.tipoVehiculoSelected.imagen.split('.')[1];
      let fileTransformed = this.blobToFile(data, this.tipoVehiculoSelected.imagen, extension);
  ...
  createImageFromBlob(image: Blob) {
    ...
    this.tipoVehiculoSelected.imageToShow = this._domSanitizer.bypassSecurityTrustUrl(urlCreator.createObjectURL(image));
  ...
  renderDataTable(dataTablesParameters: any, callback){
    ...
    this.http
      .post<DataTablesResponse>(
        GLOBAL.urlBackend + '/tipoVehiculo/datatable',
        dataTablesParameters, {headers}
      ).subscribe(resp => {

      console.log('Imprimir datos');
      console.log(resp);
      this.tiposVehiculo = resp.data;
  ...
  cambiarEstado(codigo, estado, event: MatSlideToggleChange) {
    this._tipoVehiculoService.cambiarEstado(codigo, estado).subscribe(
      response => {
  ...
  editar(){
    console.log('Modificando un tipo de vehiculo');

    console.log(this.firstFormGroup);
    console.log(this.selectedFile);

    let tipoVehiculoProcesado: TipoVehiculo = new TipoVehiculo(
      this.tipoVehiculoSelected.codigo,
    ...
    console.log(tipoVehiculoProcesado);

    this._tipoVehiculoService.editar(tipoVehiculoProcesado, this.selectedFile)
      .subscribe(
        response => {
          console.log(response);

          if(response.code === 200){
            console.log('Se edito correctamenteloadImageFromService');
            this.accion = 'NINGUNA';
            this.tipoVehiculoSelected = null;
  ...
  adicionar() {
    ...
    this._tipoVehiculoService.agregar(tipoVehiculoProcesado, this.selectedFile)
      .subscribe(
        response => {
          console.log(response);
  ...
  eliminar(){
    ...
    console.log('CODIGO: ' + this.tipoVehiculoSelected.codigo);

    this._tipoVehiculoService.deleteTipoVehiculo(this.tipoVehiculoSelected.codigo)
      .subscribe(
        response => {
          console.log(response);
...
// PARA SACAR LA INFORMACION DE CADA FILA
  someClickHandler(rowInfo: any): void {
    ...
    if(rowInfo != null){
      ...
      this.tipoVehiculoSelected = rowInfo;
...
// EVENTO PARA EL CLIC EN CADA FILA
  ClickRow(rowCodigo, element: any) {
    ...
    if (element === this.rowSelected) {
      ...
      this.someClickHandler(this.tiposVehiculo[rowCodigo]);
    }
