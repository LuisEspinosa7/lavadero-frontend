...
import {DialogGestionLavaderoComponent} from './dialog-gestion-lavaderos/dialog-gestion-lavadero.component';
...
@Component({
  selector: 'app-administrador-gestion-lavaderos',
  templateUrl: './gestion-lavaderos.component.html',
  styleUrls: ['./gestion-lavaderos.component.css']
})

export class GestionLavaderosComponent implements OnInit {
  ...
  rowSelected: string = '';
  public lavaderoSelected: Lavadero;
  private row;
  ...
  iniciarFormulariosParaCreacion(){
    ...
    this.firstFormGroup = this.formBuilder.group({
      ...
      nit: [null, Validators.compose([Validators.required, Validators.minLength(3), Validators.maxLength(40)])]
    })
  ...
  iniciarFormulariosParaModificar() {
    ...
    this.firstFormGroup = this.formBuilder.group({
      nombre: [this.lavaderoSelected.nombre, Validators.compose([Validators.required, Validators.minLength(3), Validators.maxLength(40)])],
      descripcion: [this.lavaderoSelected.descripcion, Validators.compose([Validators.minLength(3), Validators.maxLength(40)])],
      nit: [this.lavaderoSelected.nit, Validators.compose([Validators.required, Validators.minLength(3), Validators.maxLength(40)])]
    })
  ...
  loadImageFromService() {
    ...
    this.isImageLoading = true;
    this._imageService.getImage(this.lavaderoSelected.imagen, 'lavadero').subscribe(data => {
      console.log('Data: ');
      console.log(data);
      let extension = this.lavaderoSelected.imagen.split('.')[1];
      let fileTransformed = this.blobToFile(data, this.lavaderoSelected.imagen, extension);
  ...
  createImageFromBlob(image: Blob) {
    ...
    this.lavaderoSelected.imageToShow = this._domSanitizer.bypassSecurityTrustUrl(urlCreator.createObjectURL(image));
  ...
  renderDataTable(dataTablesParameters: any, callback){
    ...
    this.http
      .post<DataTablesResponse>(
        GLOBAL.urlBackend + '/lavadero/datatable',
        dataTablesParameters, {headers}
      ).subscribe(resp => {

      console.log('Imprimir datos');
      console.log(resp);
      this.lavaderos = resp.data;
  ...
  cambiarEstado(codigo, estado, event: MatSlideToggleChange) {
    this._lavaderoService.cambiarEstado(codigo, estado).subscribe(
      response => {
  ...
  editar(){
    console.log('Modificando un lavadero');

    console.log(this.firstFormGroup);
    console.log(this.selectedFile);

    let lavaderoProcesado: Lavadero = new Lavadero(
      this.lavaderoSelected.codigo,
      this.firstFormGroup.controls['nombre'].value,
      this.firstFormGroup.controls['descripcion'].value,
      null,
      null,
      null,
      this.firstFormGroup.controls['nit'].value);

    console.log('Lavadero');
    console.log(lavaderoProcesado);

    this._lavaderoService.editar(lavaderoProcesado, this.selectedFile)
      .subscribe(
        response => {
  ...
  adicionar() {
    ...
    console.log('Adicionando un tipo de vehiculo');
    ...
    this._lavaderoService.agregar(lavaderoProcesado, this.selectedFile)
      .subscribe(
        response => {
          console.log(response);
  ...
  eliminar(){
    ...
    console.log('CODIGO: ' + this.lavaderoSelected.codigo);

    this._lavaderoService.delete(this.lavaderoSelected.codigo)
      .subscribe(
        response => {
          console.log(response);
...
// EVENTO PARA EL CLIC EN CADA FILA
  ClickRow(rowCodigo, element: any) {
    ...
    if (element === this.rowSelected) {
      ...
      this.someClickHandler(this.lavaderos[rowCodigo]);
    }
